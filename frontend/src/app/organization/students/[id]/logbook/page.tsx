...
import { getOrgStudentLogbook, addOrgComment } from "@/lib/api";
import { LogbookEntry } from "@/lib/types";

export default function OrgStudentLogbookPage() {
  ...
  const loadLogbook = useCallback(async () => {
    ...
    try {
      const data = await getOrgStudentLogbook(session.accessToken, studentId);
  ...
  async function handleComment(entryId: string) {
    ...
    setSubmitting(true);
    try {
      await addOrgComment(session.accessToken, entryId, {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {groupedByWeek[week].map((entry) => (
                <div key={entry.id} style={{ ...cardStyle, borderLeft: `4px solid ${entry.status === "DRAFT" ? "#F59E0B" : "#10B981"}` }}>
                  ...
                  {entry.status === "SUBMITTED" && !entry.organizationComment && (
                    <div style={{ marginTop: 8 }}>
                      {commentEntryId === entry.id ? (
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          ...
                          <input
                            type="text"
                            placeholder="Your name and title (e.g. John Doe — IT Supervisor)"
                            value={signatureText}
